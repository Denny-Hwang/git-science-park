...
이 프로젝트는 과학사의 핵심 실험 **70개**를 "조작 → 관찰 → 해석"의 흐름으로 직접 다루며 배우는 인터랙티브 학습 플랫폼입니다. 누구나 새 실험을 추가하거나 기존 실험을 개선할 수 있도록 열려 있습니다. 물리학 지식, 프런트엔드 기술, 글쓰기, 디자인 감각 — 어떤 형태의 기여든 환영합니다.
...
| 유형 | 설명 | 예시 |
| --- | --- | --- |
| 🧪 **새 실험** | 새 실험을 제안·구현하거나 `experiments.json`에 `planned` 상태로 등록된 항목을 구현 | 새 물리 실험 페이지 작성 |
