...
- **DATA-MODEL.md 현행화** — 규범 스키마 문서의 개수 불변식(8/53→9/70), 카테고리 표(09-quantum-computing 추가),
  분포 표(카테고리별 id가 비연속일 수 있음을 명시)를 실제 데이터에 맞게 갱신하고,
  전체 데이터 절은 v1.0 역사적 스냅숏임을 명시(단일 진실 공급원은 `src/data/experiments.json`).
